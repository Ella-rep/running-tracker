...
# Installation projet + build Symfony.
RUN mkdir -p /tmp/remote-app \
    ...
    && if [ "$INSTALL_REMOTE_PROJECT" = "true" ]; then \
        ...
        && GIT_SSL_NO_VERIFY=true git clone --single-branch --branch "$branch_project_name" \
                         "$CLONE_URL" \
                         /tmp/remote-app \
                && rsync -a --delete /tmp/remote-app/ /var/www/app/ \
        && if [ -n "$project_env" ] \
                        && [ -f "/var/www/app/.env.${project_env}" ]; then \
                             cp -p "/var/www/app/.env.${project_env}" /var/www/app/.env; \
           elif [ -n "$project_env" ] \
                            && [ -f "/var/www/app/deploiement/ucn/appli/.env.${project_env}" ]; then \
                             cp -p "/var/www/app/deploiement/ucn/appli/.env.${project_env}" /var/www/app/.env; \
           fi \
                && if [ -f /var/www/app/bin/console ]; then chmod +x /var/www/app/bin/console; fi \
                && if [ -f /var/www/app/package.json ]; then cd /var/www/app && /usr/bin/npm run build; fi; \
      fi \
